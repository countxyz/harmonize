...
class TaskForm
  ...
  def fade_in_new_task_form
    Element.find('#new-task-form').effect :fade_in
  end

  def fade_out_new_task_form
    Element.find('#new-task-form').effect :fade_out
  end

  def show_task_form_new
    new_task.on :click do |event|
